...
//choi read dht every minute
/*            if (rtc.getsecond()== 10) {                     // scroll temp/humidity
                f_scroll_x0 = true;
                _humid = dht.readHumidity();
                _temp = dht.readTemperature();
                if (isnan(_humid) || isnan(_temp)) {
                    _humid = -100;          // set invalid value if error
                    _temp = -100;
                }
            } */
...
//++choi display dot every 1sec for 500ms
