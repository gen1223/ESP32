...
// Version 1.2.1
//
//  Choi ++20200213 build Home NTP Clock using Max7219 x 8
//        - Adjust brightness in every 2 second using Cds
//        - Display temp/humidity by using DHT22 in every minute
//  Choi ++20200326 add codes for reliability of WiFi Connection
//        - Max7219 x 4, display dot every 500ms
//  Choi ++20201105 add codes for BME280, display temp, humid, pressure
//  Choi ++20201116 test to fixup the bme reset problem.. change power source to GPIO-out
//  Choi ++20201117 add codes to display by using switch
//  Choi ++20201120 BME280 gives invalid values after a few days. add reset if invalid value
//  Choi ++20201124 add interrupt service routine for switch
//  Choi ++20201231 change sensor to bme680
//  Choi ++20210301 bug fix: tSW is not reset.. add %60
//                  reset wifi every midnight and sync ntp
//  Choi ++20210303 change wifi init scheme
//                  rtc.begin() should be only once.. sntp_stop(), sntp_init() ??
//  Choi ++20210304 adjust re-sync time (6h) -> 2h -> 1h
//
//
...
// Cds
#define CDS           33      // adc2 can't be used with wifi!!!

// Credentials ----------------------------------------
//#define SSID         "soft";                      // Your WiFi credentials here
//#define PW           "soft0180";
#define SSID         "KAU-Guest";                      // Your WiFi credentials here
#define PW           "";
//#define SSID         "myLGNet_Eagle_2";                      // Your WiFi credentials here
//#define PW           "NavVision6022!";
//#define SSID         "Candy";                      // Your WiFi credentials here
//#define PW           "papabear";

// Timezone -------------------------------------------
#define TZName       "KST-9"                          // Seoul
...
// User defined text ----------------------------------
#define UDTXT        " Rejoice always. "  // Pray without ceasing. Give thanks in everything.     "
...
// other defines --------------------------------------
#define BRIGHTNESS   4     // values can be 0...15
...
#define BME_Power   19      // choi++20201116
#define TL_LOW      -40     // choi++20201120 temp low limit
#define TL_HIGH     85      // temp high limit
#define PL_LOW      300     // press low limit
#define PL_HIGH     1100    // press high limit
#define buttPin     15
Adafruit_BME680 bme; // I2C
int bmeID;
int tSW;              // choi++20201117

//-----------------------------------------------------
//global variables
//String   _SSID = "";
//String   _PW   = "";
String   _SSID = SSID;                  // choi++
String   _PW   = PW;                    // choi++
...
boolean  _f_tckr500ms0 = false;          //++choi
boolean  _f_tckr500ms1 = false;          //++choi
...
uint16_t _chbuf[256];
float    _temp, _humid, _press, _gas;          //++choi
...
const uint8_t font_t[96][9] = { // font only for the time
        ...
        { 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },   // 0x3b, blank ++choi
...
const uint8_t font_p[346][9] = { // proportional font
        ...
        //--choi make full-space 5dot width '@'
        /*0032*/ /* { 0x05, 0x0e, 0x11, 0x17, 0x15, 0x17, 0x10, 0x0f, 0x00 },   // 0x0040, @    COMMERCIAL AT */
        /*0032*/ { 0x05, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 },   // 0x0040, @    ' '
...
//--choi    if ((k >= 0) && (k < 11)){           //character found in font?
    if ((k >= 0) && (k < 12)){           //character found in font?
...
//++choi add 0.5sec
    ...
    if (cnt50ms == 20) {
        ...
        cnt1s++;
        cnt50ms = 0;
        _f_tckr500ms1 = true;   //++choi
    }
    if (cnt1s == 3600) { // 1h
        cnt1h++;
        cnt1s = 0;
    }
    if (cnt1h == 1 /*24*/) { // 1d -> 6h choi++20210304 -> 2h choi++20210311 -> 20210315
        _f_tckr24h = true;
        cnt1h = 0;
    }
            /* Temp!! choi++20210303 test
            if (cnt50ms == 0 && (cnt1s%60) == 0) {
                _f_tckr24h = true;
            } */
}

/* ++choi test */
void disp_str(int16_t posX, String txt) {
    int16_t p=0, i=0;
    while(txt[i]){
        p+=char2Arr_p(txt[i] - 0x20, posX - p);
        p+=char2Arr_p(0, posX - p);     // 1px space
        if(txt[i++] == 0x20) p+=2;      // +2px space
    }
}

//choi.. convert cds value to brightness
int getBr(int vCds) {
    Serial.println(vCds);
    if (vCds > 500) return 15;
//    if (vCds < 5) return 0;
    return (vCds * 15) / 500;
}

//choi++20201124 button interrupt service routine
void IRAM_ATTR pressButt() {
    if (tSW != 0xff) {
        Serial.println("button-press duplicate.. Ignore..");
        return;
    }
    else {
        tSW = (rtc.getsecond()+1)%60;
        Serial.print("Button Pressed: ");
        Serial.println(tSW);
    }
}

//++20201231 read bm2680
void getBME680Readings(){
  ...
  _press = bme.pressure / 100.0;
  _humid = bme.humidity;
  _gas = bme.gas_resistance / 1000.0;
}

#define WTOUT 10    // timeout 10sec
//choi++20210303 initialize WiFi
//  Return 0 : Connected
...
//    dht.begin();              //++choi
/*    delay(100);
    bool status = bme.begin(0x76);   //++choi
    if (!status) {
...
/*    while (!bme.begin(0x76)) {
    ...
    while (!bme.begin()) {
        Serial.println("BME680 will be reset by SW..");
...
//++choi 20201231
    ...
    delay(50);
    getBME680Readings();   
/*    bmeID = bme.sensorID();         // save sendorID
    Serial.print("\nBME-ID: ");
    Serial.println(bmeID);  --choi */
    
    tSW = 0xff;                     // choi++20201117
    pinMode(buttPin, INPUT);
    // ++20201124 setup interrupt SR
    // PIR Motion Sensor mode INPUT_PULLUP
    attachInterrupt(digitalPinToInterrupt(buttPin), pressButt, FALLING /*RISING*/);   // choi++Temp!
...
/* choi--20210303 change init wifi scheme
    WiFi.disconnect();
    WiFi.mode(WIFI_MODE_STA);
    _SSID=SSID;
    _PW=PW;
//choi++20200326
    WiFi.begin(_SSID.c_str(), _PW.c_str()); 
//    wifiMulti.addAP(_SSID.c_str(), _PW.c_str()); 
...
//choi++20200326
    while (WiFi.status() != WL_CONNECTED){
        delay(1000);
        Serial.print(".");
//choi--20210302        WiFi.begin(_SSID.c_str(), _PW.c_str());
    }
choi-- */

    while (initWiFi()) ;        // choi++20210303 change init wifi scheme
    
//choi--20210302    if(WiFi.status()==WL_CONNECTED){
//    if(wifiMulti.run()==WL_CONNECTED){
        _myIP=WiFi.localIP().toString();
        Serial.printf("WiFI_info  : WiFi connected\n");
        Serial.printf("WiFI_info  : IP address %s\n", _myIP.c_str());
//choi--20210302    }else{
//choi--20210302        Serial.printf("WiFi credentials are not correct\n");
//--choi        _SSID = ""; _myIP="0.0.0.0";
//choi--20210302        while(true){;} // endless loop
//choi--20210302    }
...
{   
    ...
    boolean f_scroll_x0 = false;      //++choi
    boolean f_scroll_x1 = false;
    boolean f_scroll_x2 = false;
    int f_disp_thp = 0;               //++choi for temp, humid, pressure
...
/* choi-- change init wifi scheme
//choi++20210301 reset wifi every day
            WiFi.disconnect();
            WiFi.mode(WIFI_MODE_STA);
            WiFi.begin(_SSID.c_str(), _PW.c_str()); 
            Serial.println("WiFI_info  : Connecting WiFi...");
...
//choi--20210303 this makes system reboot            _f_rtc= rtc.begin(TZName);
//            if(!_f_rtc) Serial.println("no timepacket received from ntp");
            Serial.println("Re-sync NTP...");
            sntp_stop();
            sntp_init();            // choi++ resync
...
//choi++20201116 check bme sensor connectivity .. 
          if (/*bme.sensorID() != bmeID || */ //--choi 20201231 
              _temp < TL_LOW || _temp > TL_HIGH ||
              _press < PL_LOW || _press > PL_HIGH) {          // choi++20201120 check invalid value
/*              while (!bme.begin(0x76)) {
                  Serial.println("BME280 will be reset by SW..");
                  digitalWrite(BME_Power, LOW);
                  delay(50);
...
//choi++20201117 check switch --20201124 move code to ISR
/*          if (tSW == 0xff && digitalRead(buttPin)) {
              tSW = (rtc.getsecond()+1)%60;
              Serial.print("Button Pressed: ");
              Serial.println(tSW);
          } */
...
//choi adjust brightness every 2 second 
/*            if (rtc.getsecond() & 1) {
                int vC = analogRead(CDS);
                max7219_set_brightness(getBr(vC));
            } */
            
//choi read bme every minute
            if (rtc.getsecond()== 0) {    
/*                _humid = bme.readHumidity();
                _temp = bme.readTemperature();
                _press = bme.readPressure() / 100.0F; */
                getBME680Readings();
                Serial.print(".");
                Serial.println(_temp);    //choi 20210301
            } 

//--choi            if (rtc.getsecond() == 30) f_scroll_x1 = true;  // scroll ddmmyy
#ifdef UDTXT
//--choi            if (rtc.getsecond() == 45) f_scroll_x2 = true;  // scroll userdefined text
#endif //UDTXT

            //++choi display temp, humid
            int tsec = rtc.getsecond();
            if (tSW != 0xff) {            // choi++20201117
                if (tsec == tSW) f_disp_thp = 1;
                if (tsec == (tSW+2)%60) f_disp_thp = 2;   // choi++20210301 mod 60
                if (tsec == (tSW+4)%60) f_disp_thp = 3;
                if (tsec == (tSW+6)%60) f_disp_thp = 4;
                if (tsec >= (tSW+8)%60) {f_disp_thp = 0; tSW = 0xff;}
            }
            else {
                if (tsec == 5 || tsec == 25 || tsec == 45) f_disp_thp = 1;    // temp
                if (tsec == 7 || tsec == 27 || tsec == 47) f_disp_thp = 2;    // humid
                if (tsec == 9 || tsec == 29 || tsec == 49) f_disp_thp = 3;    // pressure
                if (tsec == 11 || tsec == 31 || tsec == 51) f_disp_thp = 4;    // gas
                if (tsec == 13 || tsec == 33 || tsec == 53) f_disp_thp = 0;   // no display
            }
        } // end 1s

//++choi.20201112 display temp/humid/pressure
            String txt;
            switch (f_disp_thp) {
            case 1:
                txt = String(_temp,1) + " C";
                txt[txt.indexOf(' ')] = (byte)(0x20+112);   // degree font
                break;
            case 2:
                txt = String(_humid,1) + "%@@";
                break;
            case 3:
                txt = String(_press,0) + "hPa";
                break;
            case 4:
                txt = String(_gas,1) + "k @";               //++choi
                txt[txt.indexOf(' ')] = (byte)(0x20+216);   // degree ohm choi++20201231
                break;
            }
            disp_str(_maxPosX, txt);
...
//          -------------------------------------
            if (f_scroll_x0 == true) {
                ...
                if (_zPosX == _maxPosX){f_scroll_x0 = false; _dPosX = -8;}
            }
//          -------------------------------------
            if (f_scroll_x1 == true) {
                _zPosX++;
...
//          -------------------------------------
//++choi disp time when thp is not displayed
        if (f_disp_thp == 0) {
          
            if (sc1 == 1) {
                if (_f_updown == 1) y--;
                else                y++;
                char2Arr_t(48 + sek12, _zPosX - 40/*42*/, y);
                char2Arr_t(48 + sek11, _zPosX - 40/*42*/, y + y1);
                if (y == 0) {sc1 = 0; f_scrollend_y = true;}
            }
            else char2Arr_t(48 + sek1, _zPosX - 40/*42*/, 0);
//          -------------------------------------
            if (sc2 == 1) {
                char2Arr_t(48 + sek22, _zPosX - 34/*36*/, y);
                char2Arr_t(48 + sek21, _zPosX - 34/*36*/, y + y1);
                if (y == 0) sc2 = 0;
            }
            else char2Arr_t(48 + sek2, _zPosX - 34/*36*/, 0);

            if (_f_tckr500ms0 == true) {
                char2Arr_t(':', _zPosX - 30/*32*/, 0);
                _f_tckr500ms0 = false;
            }
            if (_f_tckr500ms1 == true) {
                char2Arr_t(':'+1, _zPosX - 30/*32*/, 0);    // blank font after ':'
                _f_tckr500ms1 = false;
            }
            
//          -------------------------------------
            if (sc3 == 1) {
                char2Arr_t(48 + min12, _zPosX - 23/*25*/, y);
                char2Arr_t(48 + min11, _zPosX - 23/*25*/, y + y1);
                if (y == 0) sc3 = 0;
            }
            else char2Arr_t(48 + min1, _zPosX - 23/*25*/, 0);
//          -------------------------------------
            if (sc4 == 1) {
                char2Arr_t(48 + min22, _zPosX - 17/*19*/, y);
                char2Arr_t(48 + min21, _zPosX - 17/*19*/, y + y1);
                if (y == 0) sc4 = 0;
            }
            else char2Arr_t(48 + min2, _zPosX - 17/*19*/, 0);
            char2Arr_t(':', _zPosX - 13/*15*/ + x, 0);

//          -------------------------------------
            if (sc5 == 1) {
                char2Arr_t(48 + std12, _zPosX - 6/*8*/, y);
                char2Arr_t(48 + std11, _zPosX - 6/*8*/, y + y1);
                if (y == 0) sc5 = 0;
            }
            else char2Arr_t(48 + std1, _zPosX - 6/*8*/, 0);
//          -------------------------------------
            if (sc6 == 1) {
...
//++choi
/*            if (!f_scroll_x0 && !f_scroll_x1 && !f_scroll_x2)
                scrolltext(16, WD_arr[rtc.getweekday()]);  */

//++choi 
/*            if(f_scroll_x0){            // temp/humidity
                if (_humid != -100 && _temp != -100) {   // display only when valid
                    String txt = "   Temp: ";
                    txt += String(_temp,1) + "°C, Humid: " + String(_humid,1) + "%   ";
