...
/*    bmeID = bme.sensorID();         // save sendorID
    ...
    attachInterrupt(digitalPinToInterrupt(buttPin), pressButt, RISING);
...
//--choi            if (rtc.getsecond() == 45) f_scroll_x2 = true;  // scroll userdefined text
            ...
            if (tSW != 0xff) {            // choi++20201117
                ...
                if (tsec == tSW+2) f_disp_thp = 2;
                if (tsec == tSW+4) f_disp_thp = 3;
                if (tsec == tSW+6) f_disp_thp = 4;
                if (tsec == tSW+8) {f_disp_thp = 0; tSW = 0xff;}
            }
            else {
