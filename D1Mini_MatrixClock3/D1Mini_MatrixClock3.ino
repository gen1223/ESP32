...
//  Choi ++20201230 modify for D1Mini (esp8266)
//
//
...
//#include <WiFiMulti.h>
#include "ESP8266WiFi.h"        // ++choi
//#include <SPI.h>
#include "SPI.h"                /++choi for 8266
//#include <Ticker.h>
#include "Ticker.h"             /++choi for 8266
#include <time.h>
#include <Wire.h>
...
// Digital I/O used for 8266
#define SPI_MOSI      13
#define SPI_MISO      12    // not connected
#define SPI_SCK       14
#define MAX_CS        15
...
#define BME_Power   1       // choi gpio 1
...
uint16_t _chbuf[256];
float    _temp, _humid, _press;          //++choi
...
//--choi        vTaskDelay(uint16_t(2000 / portTICK_PERIOD_MS));
        delay(2000);
        time(&now);
...
//    digitalWrite(MAX_CS, HIGH);
//--choi    SPI.begin(SPI_SCK, SPI_MISO, SPI_MOSI);
    if (!SPI.pins(SPI_SCK, SPI_MISO, SPI_MOSI, MAX_CS))
...
//--choi            if (rtc.getsecond() == 45) f_scroll_x2 = true;  // scroll userdefined text
            ...
            if (tSW != 0xff) {            // choi++20201117
                ...
                if (tsec == tSW+6) {f_disp_thp = 0; tSW = 0xff;}
            }
            else {
                ...
                if (tsec == 11 || tsec == 31 || tsec == 51) f_disp_thp = 0;   // no display
            }
        } // end 1s
